...
public class GreetResource
{
	 ...
	 */
	@Inject
	public GreetResource(final GreetingProvider greetingConfig)
	{
	...
	public Response updateGreeting(final JsonObject jsonObject)
	{
		...
		{
			final JsonObject entity = JSON.createObjectBuilder().add("error", "No greeting provided").build();
			return Response.status(Response.Status.BAD_REQUEST).entity(entity).build();
		}

		final String newGreeting = jsonObject.getString("greeting");

		this.greetingProvider.setMessage(newGreeting);
		return Response.status(Response.Status.NO_CONTENT).build();
	}

	private JsonObject createResponse(final String who)
	{
		final String msg = String.format("%s %s!", this.greetingProvider.getMessage(), who);
